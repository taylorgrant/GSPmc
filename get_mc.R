...
pacman::p_load(tidyverse, janitor, here, glue, rvest, crayon)
...
cat(crayon::cyan("Pulling all links from MC...\n"))
tmp <- extract_links()
...
cat(crayon::cyan("Finding the pdfs. This takes a minute...\n"))
pdflinks <- tmp$link_base %>%
  map(extract_crosstab_links)

# drop NAs on failed extract
pdflinks <- pdflinks[!is.na(pdflinks)]
...
# download the pdfs
cat(crayon::cyan("Downloading to file...\n"))
walk2(pdflinks$pdf_link, pdflinks$filenumber, pdf_download)
...
# BREAKOUT TEXT AND KEYWORDS [skip for now] ----------------------------------
